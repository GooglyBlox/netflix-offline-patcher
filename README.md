...
The oldest SDK also appears in **non-Unity** games. Unity titles reach it through a `com.netflix.unity` bridge; **GameMaker** titles (engine `libyoyo.so`) instead have a game-authored GameMaker extension class that implements `NetflixSdkEventHandler` and exposes `Nfxa*` methods that GML calls. When there is no Unity bridge, the tool detects the GameMaker case, finds that glue class, and patches it: it delivers a synthetic authenticated state to the glue's event receiver (so the game's own handler reports "signed in"), and redirects the game's cloud-save slot methods to the same local file store, reusing the game's own result callbacks (a slot miss must report "unknown slot" so the game starts fresh rather than trying to parse empty save data). GameMaker's engine is ARM-only, so the tool also strips the engine-less `x86`/`x86_64` lib folders, which lets x86 emulators run the app under ARM translation instead of failing to find the engine.
