...
# -----------------------------------------------------------------------------
# Unreal Engine 4 + oldest SDK (com.netflix.android.api), no Unity/GameMaker bridge.
#
# UE4 Netflix titles (e.g. CoComelon: Play with JJ) carry the same oldest SDK but bridge it to
# native C++ (libUE4.so) through a game glue class built on Unreal's JNI convention: "Thunk_*"
# methods the native side CALLS in, and "native nativeOn*" methods that call BACK into native.
# Auth flow: native calls Thunk_checkNetflixUserAuth(nativeObj); the glue posts a Runnable that
# runs NetflixSdk.checkUserAuth(activity) (a network handshake); the result arrives via the
# registered NetflixSdkEventHandler.onUserStateChange(state), which calls
# nativeOnNetflixUserStateChanged(nativeObj, previous, current, playerIdentity) - a non-null
# current profile == signed in. Offline the handshake never delivers a profile, so the game sits
# on the Netflix gate. These titles keep progress in their OWN engine-local save (not the Netflix
# cloud), so - unlike the other gen-0 games - there is no slot store to build; auth is the only wall.
#
# We discover the glue and its two inner classes (the checkUserAuth Runnable and the event
# handler) by parsing, and rewrite both to deliver a synthetic signed-in state straight to the
# native callback, no network:
#   1. SdkErrorActivity$Companion.startSdkErrorActivity(...) -> return-void
#   2. a dummy offline PlayerIdentity in the glue's package (never null)
#   3. the checkUserAuth Runnable's run() -> deliver (nativeObj, null, offline-profile, player)
#   4. the event handler's onUserStateChange(state) -> deliver the same (a chokepoint, so a live
#      signed-out event can't undo us)
# The native-pointer field is flipped to public so the inner classes can read it directly, which
# avoids depending on the compiler-assigned access$NNN accessor numbers.
# -----------------------------------------------------------------------------
